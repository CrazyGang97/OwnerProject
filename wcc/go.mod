module gang_wcc

go 1.19

require (
	code.byted.org/webcast/rpcv2_webcast_platform_wcc v0.0.0-20220824065321-78cb4ac717b6
	code.byted.org/webcast/wcc_codegen/webcast_allowlist_core v0.0.5
	code.byted.org/webcast/wcc_codegen/webcast_wcc_gangtest4 v0.0.1
	code.byted.org/webcast/wccclient v0.1.2
)

require (
	code.byted.org/bytedance/tos-cloud-client v1.3.0 // indirect
	code.byted.org/bytedtrace-contrib/kitex-go v1.1.33 // indirect
	code.byted.org/bytedtrace/bytedtrace-client-go v1.0.47 // indirect
	code.byted.org/bytedtrace/bytedtrace-common/go v0.0.12 // indirect
	code.byted.org/bytedtrace/bytedtrace-conf-provider-client-go v0.0.18 // indirect
	code.byted.org/bytedtrace/interface-go v1.0.20 // indirect
	code.byted.org/bytedtrace/serializer-go v1.0.0 // indirect
	code.byted.org/golf/consul v2.1.13+incompatible // indirect
	code.byted.org/gopkg/asynccache v0.0.0-20210422090342-26f94f7676b8 // indirect
	code.byted.org/gopkg/consul v1.2.2 // indirect
	code.byted.org/gopkg/ctxvalues v0.4.0 // indirect
	code.byted.org/gopkg/debug v0.10.0 // indirect
	code.byted.org/gopkg/env v1.4.19 // indirect
	code.byted.org/gopkg/etcd_util v2.3.3+incompatible // indirect
	code.byted.org/gopkg/etcdproxy v0.1.1 // indirect
	code.byted.org/gopkg/logid v0.0.0-20211104042040-f78600e482f2 // indirect
	code.byted.org/gopkg/logs v1.2.13 // indirect
	code.byted.org/gopkg/logs/v2 v2.1.26 // indirect
	code.byted.org/gopkg/metainfo v0.1.1 // indirect
	code.byted.org/gopkg/metrics v1.4.14 // indirect
	code.byted.org/gopkg/metrics/v3 v3.1.22 // indirect
	code.byted.org/gopkg/net2 v1.4.1 // indirect
	code.byted.org/gopkg/stats v1.0.1 // indirect
	code.byted.org/gopkg/tccclient v1.4.4 // indirect
	code.byted.org/gopkg/thrift v1.6.1 // indirect
	code.byted.org/gopkg/tos v1.4.11 // indirect
	code.byted.org/kite/endpoint v3.7.5+incompatible // indirect
	code.byted.org/kite/kitex v1.8.0 // indirect
	code.byted.org/log_market/gosdk v0.0.0-20220328031951-809cbf0ba485 // indirect
	code.byted.org/log_market/loghelper v0.1.9 // indirect
	code.byted.org/log_market/tracelog v0.1.4 // indirect
	code.byted.org/log_market/ttlogagent_gosdk v0.0.3 // indirect
	code.byted.org/log_market/ttlogagent_gosdk/v4 v4.0.26 // indirect
	code.byted.org/middleware/fic_client v0.2.0 // indirect
	code.byted.org/middleware/mcache v0.0.0-20210324103046-d91c09d6b541 // indirect
	code.byted.org/security/go-spiffe-v2 v1.0.3 // indirect
	code.byted.org/security/sensitive_finder_engine v0.3.16 // indirect
	code.byted.org/security/zti-jwt-helper-golang v1.0.13 // indirect
	code.byted.org/service_mesh/shmipc v0.2.3 // indirect
	code.byted.org/shield/balancer v0.0.0-20210128081154-01fb65e45023 // indirect
	code.byted.org/shield/common v1.0.11 // indirect
	code.byted.org/toutiao/easygo v0.2.1 // indirect
	code.byted.org/trace/trace-client-go v1.3.6 // indirect
	code.byted.org/ttarch/cds_common v0.2.14 // indirect
	code.byted.org/ttarch/cds_sdk v0.2.15 // indirect
	code.byted.org/webcast/rpcv2_webcast_overpass_common_idl v0.0.0-20220824063531-df3038f378df // indirect
	github.com/Knetic/govaluate v3.0.1-0.20171022003610-9aa49832a739+incompatible // indirect
	github.com/StackExchange/wmi v0.0.0-20210224194228-fe8f1750fd46 // indirect
	github.com/apache/thrift v0.13.0 // indirect
	github.com/bytedance/gopkg v0.0.0-20220623074550-9d6d3df70991 // indirect
	github.com/cespare/xxhash v1.1.0 // indirect
	github.com/choleraehyq/pid v0.0.15 // indirect
	github.com/cloudwego/kitex v0.2.0 // indirect
	github.com/cloudwego/netpoll v0.2.0 // indirect
	github.com/cloudwego/netpoll-http2 v0.0.6 // indirect
	github.com/cloudwego/thriftgo v0.1.3 // indirect
	github.com/go-kit/log v0.2.1 // indirect
	github.com/go-logfmt/logfmt v0.5.1 // indirect
	github.com/go-ole/go-ole v1.2.5 // indirect
	github.com/gogo/protobuf v1.3.2 // indirect
	github.com/golang/protobuf v1.5.2 // indirect
	github.com/google/uuid v1.3.0 // indirect
	github.com/gorilla/mux v1.8.0 // indirect
	github.com/json-iterator/go v1.1.12 // indirect
	github.com/klauspost/compress v1.15.8 // indirect
	github.com/modern-go/concurrent v0.0.0-20180306012644-bacd9c7ef1dd // indirect
	github.com/modern-go/reflect2 v1.0.2 // indirect
	github.com/opentracing/opentracing-go v1.2.0 // indirect
	github.com/pkg/errors v0.9.1 // indirect
	github.com/shirou/gopsutil v3.21.6+incompatible // indirect
	github.com/sirupsen/logrus v1.9.0 // indirect
	github.com/tidwall/gjson v1.9.3 // indirect
	github.com/tidwall/match v1.1.1 // indirect
	github.com/tidwall/pretty v1.2.0 // indirect
	github.com/volcengine/ve-tos-golang-sdk v0.2.2 // indirect
	github.com/zeebo/errs v1.3.0 // indirect
	golang.org/x/crypto v0.0.0-20220622213112-05595931fe9d // indirect
	golang.org/x/net v0.0.0-20220708220712-1185a9018129 // indirect
	golang.org/x/sync v0.0.0-20220601150217-0de741cfad7f // indirect
	golang.org/x/sys v0.0.0-20220715151400-c0bba94af5f8 // indirect
	golang.org/x/text v0.3.7 // indirect
	golang.org/x/time v0.0.0-20220609170525-579cf78fd858 // indirect
	google.golang.org/genproto v0.0.0-20220718134204-073382fd740c // indirect
	google.golang.org/grpc v1.48.0 // indirect
	google.golang.org/protobuf v1.28.0 // indirect
	gopkg.in/square/go-jose.v2 v2.6.0 // indirect
	gopkg.in/yaml.v3 v3.0.0-20210107192922-496545a6307b // indirect
)
